...
import { sendNotificationToUser, sendNotificationToMultipleUsers, NotificationPayload } from '@/lib/firebase-admin';
...
export async function POST(request: NextRequest) {
  try {
    ...
    if (targetUserId) {
      ...
      result = await sendNotificationToUser(fcmToken, payload);
      ...
      result = await sendNotificationToMultipleUsers(fcmTokens, payload);
    } else {
      // 모든 사용자에게 알림 전송
